...
import { Plus, Users, MessageSquare, Clock, GripVertical, X, CheckSquare, Tag, Trash2, ChevronDown, MoreHorizontal } from 'lucide-react';
import {
  DndContext,
  closestCenter,
  KeyboardSensor,
...
const SortableCard = ({ card, onClick, isGuestMode }) => {
  ...
  return (
    <div
      ...
      onClick={() => onClick(card)} // ИЗМЕНИЛИ: убрали проверку isGuestMode
    >
...
const SortableList = ({ list, onAddCard, onCardClick, onDeleteList, currentUserRole, isGuestMode }) => {
  const {
    attributes,
    listeners,
    setNodeRef,
...
const ProjectDetail = () => {
  ...
  const handleAddCardClick = (list) => {
    if (isGuestMode || currentUserRole === 'viewer') {
      alert('Viewers cannot create cards');
      return;
    }
  ...
  try {
    ...
    // ДОБАВЬТЕ ЭТОТ БЛОК ДЛЯ УВЕДОМЛЕНИЙ
    // Создаем уведомления для назначенных пользователей
    
    // КОНЕЦ БЛОКА ДЛЯ УВЕДОМЛЕНИЙ
  ...
  try {
    ...
    // ДОБАВЬТЕ ЭТОТ БЛОК ДЛЯ УВЕДОМЛЕНИЙ ПРИ НАЗНАЧЕНИИ
...
const handleRemoveAssignee = async (cardId, assigneeId) => {
      ...
      <div className="flex justify-between items-start">
        <div>
          <div className="flex items-center space-x-2 mb-2">
            ...
            {isGuestMode && (
              <span className="text-blue-600 text-sm">Guest View Mode</span>
            )}
          </div>
          <h1 className="text-2xl font-bold text-gray-900">{project.name}</h1>
          <p className="text-gray-600">{project.description}</p>

          {/* === ВСТАВЬТЕ ЭТОТ БЛОК ПРЯМО ЗДЕСЬ === */}
          ...
          <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm text-blue-700">
                You are viewing this project as a guest. You can view cards but cannot make changes.
              </span>
            </div>
          </div>
        )}
  ...
  {selectedCard.assignees?.map(assignee => (
    <div key={assignee.id} className="flex items-center justify-between p-2 bg-gray-50 rounded">
      ...
      <button
        onClick={() => handleRemoveAssignee(selectedCard.id, assignee.id)}  // ← Передаем оба ID
